...
全局锁是对整个数据库实例加锁，加锁后整个实例处于只读状态。

典型应用是做全库的逻辑备份，对所有的表进行锁定，从而获取一致性视图，保证数据的完整性。

## 表级锁

表锁：包括表共享读锁、表独占写锁。
- 共享锁（读锁）：自己读，不能写。其他事务可以读，但不能写。
- 排他锁（写锁）：自己读，也能写。其他事务不能读取，也不能写。


元数据锁：元数据是指表结构，当表有活动事务的时候，不能修改表结构，即元数据。
- 当对一张表进行增删改查的时候，自动加MDL读锁（共享）
- 当对表结构进行变更操作的时候，自动加MDL写锁（排他）

意向锁：为了解决行锁跟表锁的冲突，使用意向锁减少表锁的检查。
- 意向共享锁（IS）：查询语句后面加上 lock in shared mode。与表锁共享锁兼容，与表锁排他锁互斥。
- 意向排他锁（IX）：增删改语句会加。与表锁共享锁和排他锁都互斥。

## 行级锁

行锁：锁定单个行记录的锁。
- 共享锁：自己读，阻止其他事务获得相同数据集的排他锁。
- 排他锁：自己可以增删改数据，阻止其他事务获得相同数据集的共享锁和排他锁。

间隙锁：锁定索引记录间隙。

临键锁：行锁跟间隙锁的组合。

默认情况下，InnoDB 在可重复读的隔离级别下，使用临键锁进行搜索和索引扫描，以防止幻读。
- 针对唯一索引进行检索的时候，对已存在的记录进行等值匹配时，将会自动优化成行锁。
- InnoDB 的行锁是针对于索引加的锁，不通过索引条件检索数据，那么 InnoDB 将对表中的所有记录加锁，升级为表锁。
